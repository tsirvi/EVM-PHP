...
Class Master extends DBConnection {
	...
	function save_hall(){
		...
		if($check->num_rows > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " location Code already exists.";
		}else{
			$save = $this->conn->query($sql);
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = " location has successfully added.";
				else
					$resp['msg'] = " location details has been updated successfully.";
	...
	function delete_hall(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," location has been deleted successfully.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
